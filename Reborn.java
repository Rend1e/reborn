...
public class Reborn {
	...
	void view() {
		try {
			...
			Scanner scanner = new Scanner (System.in);
			System.out.println("Введите поисковый запрос по индексу документа");
			index_doc = scanner.nextLine ();

			resSet = statmt.executeQuery("SELECT * FROM documents WHERE index_doc LIKE '%" + index_doc + "%';");

			while (resSet.next()) {
				int id_sql = resSet.getInt("id");
				String zagolovok = resSet.getString("zagolovok");
				String index = resSet.getString("index_doc");
